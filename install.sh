...
installXray() {
	...
	if [[ "${coreInstallType}" != "1" ]]; then
		...
		if wget --help | grep -q show-progress; then
			wget -c -q --show-progress -P /etc/v2ray-agent/xray/ "https://github.com/XTLS/Xray-core/releases/download/v1.4.2/${xrayCoreCPUVendor}.zip"
		else
			wget -c -P /etc/v2ray-agent/xray/ "https://github.com/XTLS/Xray-core/releases/download/v1.4.2/${xrayCoreCPUVendor}.zip" >/dev/null 2>&1
		fi
